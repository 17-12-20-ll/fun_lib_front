<template>
  <div class="content-warp">
    <div class="top-wrap">
      <el-select v-model="selectOneSrc" placeholder="请选择一级分类" clearable>
        <template slot="prepend">类别：</template>
        <el-option :label="o.name" :value="o.id" v-for="o in one_src" :key="o.id"></el-option>
      </el-select>
      <el-input v-model="inputTwoSrc" placeholder="请输入内容" label="组名称" clearable>
    ...
    <el-table
      ...
      <el-table-column label="操作" width="300">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            @click="handleAddEntrance(scope.$index, scope.row)">添加入口
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible" width="30%" :destroy-on-close="true">
      <div v-if="title === '查看'">
        ...
        <el-divider>排序序号:<font color="red">{{two_src_info.pos}}</font></el-divider>
        <el-divider>资源类型:{{two_src_info.src_type}}</el-divider>
        <el-divider>所属一级分类:{{two_src_info.one_src}}</el-divider>
      </div>
      <el-form :model="two_src_info" v-else-if="title === '编辑'" :rules="rules" ref="two_src_info" class="form-center">
      ...
      <el-form :model="two_src_add_info" v-else-if="title==='添加资源'" :rules="rules" ref="two_src_add_info">
...
export default {
  ...
  methods: {
    async handleView (index, row) {
      ...
      // 发送请求获取详情数据
      await this.$http.getTwoSrcDetail(row.id).then(res => {
    ...
    AddEntranceSubmit (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$http.postAddThreeSrc(qs.stringify(this.three_info)).then(res => {
            if (res.code === 200) {
    ...
    onSubmit (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$http.postUpdateTwoSrc(qs.stringify(this.two_src_info)).then(res => {
            if (res.code === 200) {
    ...
    AddSubmit (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http.postAddTwoSrc(qs.stringify(this.two_src_add_info)).then(res => {
            if (res.code === 200) {
              this.dialogFormVisible = false
              this.$message({
                message: '添加成功',
    ...
    query () {
      // 查询
      this.$http.queryTwoSrc(this.inputTwoSrc, this.selectOneSrc).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '查询成功',
            center: true,
